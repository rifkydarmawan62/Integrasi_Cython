...
if not error:
    ...
    file_input = pilih_file(title = "Pilih file Python untuk dikonversi ke file C", defaultextension = ".py", filetypes = [("Kode sumber Python", "*.py *.pyx")])
    if file_input:
        DIREKTORI_FOLDER = "/".join(file_input.split("/")[:-1])
        NAMA_FILE_OUTPUT = ".".join(file_input.split("/")[-1].split(".")[:-1]) + ".c"
        file_output = f"{DIREKTORI_FOLDER}/{NAMA_FILE_OUTPUT}"
        if system() == "Windows":
            file_input = file_input.replace("/", "\\")
            file_output = file_output.replace("/", "\\")
        PERINTAH = f"cython --output-file \"{file_output}\" -3 --embed --gdb --force --verbose \"{file_input}\""
